...
I will be working on a react app and backend moving forward for inferencing that will allow users to convert entire songs with ensembles and then reconvert sections with specific models to perfect instrumentals with a human in the loop to an extent. For those who don't want an app and wish to stick to Python I will be also writing a new inference script from scratch in order to be a bit more modular and friendly to future updates.
