...
Update on project: I have a new version that I'm training now. I have changed most areas of the neural network, with the approach now utilizing two neural networks - a vocal detector and a vocal remover, with a third neural network that will be introduced which will allow users to select regions of time and mark them as either having vocals or not having vocals, allowing for user intervention should the detector fail in certain scenarios. I have carried out experiments which shows the remover utilizes the output of the detector heavily which will allow for users to influence the actions of the remover. The new architecture is a shallow u-net with a latent space multichannel vision transformer, used for both the detector and the isolator. Code will be checked in shortly with snapshots to follow, currently running experiments to finalize architecture. I will be cleaning up the repository and including installation instructions as well as a PyInstaller executable for windows to ease the process for newcomers. I will be working on a Flask backend and React frontend for this new version as well in order to allow users to create vocal maps to influence the output of the vocal remover.
